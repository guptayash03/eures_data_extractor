...
def extract_data(element): 
    ...
    occupations= element.find_elements(By.XPATH, './/jv-se-requirement-expandable//span[contains(@class, "ecl-accordion__toggle-title")]')
    occupation = ", ".join([span.text for span in occupations])


    creation_dt = element.find_elements(By.CSS_SELECTOR, "span[id*='jv-lastModificationDate']")
    if creation_dt:
    ...
    employer = element.find_elements(By.CSS_SELECTOR, "h2[class*='ecl-u-type-heading-2 ecl-u-mt-s']")
    if employer:
      employer = employer[0].text
    else:
      employer = None 

    job_sector = element.find_elements(By.CSS_SELECTOR, "span[id*='jv-employer-sector-codes-result']")
    if job_sector:
      job_sector = job_sector[0].text
    else:
      job_sector = None

    job_type = element.find_elements(By.CSS_SELECTOR, "span[id*='jv-position-schedule-result']")
    if job_type:
      job_type = job_type[0].text
    else:
      job_type = None  

    contract_type = element.find_elements(By.CSS_SELECTOR, "span[id*='jv-position-type-code-result']")
    if contract_type:
      contract_type = contract_type[0].text
    else:
      contract_type = None

    location = element.find_elements(By.CSS_SELECTOR, "span[class*='jv-address-country']")
    if location:
      location = location[0].text
    else:
      location = None  

    city = element.find_elements(By.CSS_SELECTOR, "span[class*='jv-address-city-name']")
    if city:
      city = city[0].text
    else:
      city = None
    ...
    languages= element.find_elements(By.XPATH, './/jv-se-requirement-language//span[contains(@class, "ecl-accordion__toggle-title")]')
    language = ", ".join([span.text for span in languages])

    skills= element.find_elements(By.XPATH, './/div[contains(@id, "jv-details-job-requirements-skills-section")]//dd')
    skill = ", ".join([span.text for span in skills])

    educations = element.find_elements(By.XPATH, './/div[contains(@id, "jv-details-job-requirements-education-level-section")]//dd')
    education = ", ".join([span.text for span in educations])

    legal_ID = element.find_elements(By.CSS_SELECTOR, "dd[id*='jv-details-employer-legal-id']")
    if legal_ID:
    ...
    how_to_apply = element.find_elements(By.CSS_SELECTOR, "span[id*='jv-details-application-instruction-0']")
    if how_to_apply:  
      how_to_apply = how_to_apply[0].text
    else:
      how_to_apply = None

    phone = element.find_elements(By.CSS_SELECTOR, "li[id*='jv-details-telNumber-0-0']")
    if phone:
      phone = phone[0].text
    else:
      phone = None

    contact = element.find_elements(By.CSS_SELECTOR, "li[id*='jv-details-displayName-0']")
    if contact:
      contact = contact[0].text
    else:
      contact = None

    email = element.find_elements(By.XPATH, './/li[contains(@id, "jv-details-emails-0-0")]//a')
...
for page in range(1, num_api_calls+1):
    ...
    driver.get('https://europa.eu/eures/portal/jv-se/search?page={0}&resultsPerPage=10&orderBy=BEST_MATCH&locationCodes=nl&lang=en'.format(page))
